...
/**
 * Removes the generics from a {@link PyToKnimePortObjectConverter} for subsequent use in the framework where the
...
public final class UntypedPyToKnimePortObjectConverter implements UntypedPythonPortObjectConverter {
    ...
    private final GenericsAbsorbingConverter<?, ?, ?> m_genericsAbsorber;
    ...
    public <T extends PythonPortObjectTransfer, S extends PortObjectSpec, V extends PythonPortObjectSpecTransfer> UntypedPyToKnimePortObjectConverter(
        final PyToKnimePortObjectConverter<T, S, V> typedConverter, final PythonImplementation pythonImplementation) {
        m_genericsAbsorber = new GenericsAbsorbingConverter<>(typedConverter);
    ...
    public PortObjectSpec convertSpecFromPython(final PythonPortObjectSpecTransfer source,
        final PortObjectSpecConversionContext context) {
        return m_genericsAbsorber.convertSpecFromPython(source, context);
    }

    public PortObject convertPortObjectFromPython(final PythonPortObjectTransfer source, final PortObjectSpec spec,
        final PortObjectConversionContext context) {
        return m_genericsAbsorber.convertPortObjectFromPython(source, spec, context);
    }

    private static final class GenericsAbsorbingConverter<T extends PythonPortObjectTransfer, S extends PortObjectSpec, V extends PythonPortObjectSpecTransfer> {

        private final PyToKnimePortObjectConverter<T, S, V> m_typedConverter;

        private GenericsAbsorbingConverter(final PyToKnimePortObjectConverter<T, S, V> typedConverter) {
